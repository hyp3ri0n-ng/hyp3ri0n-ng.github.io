...
Breakpoint 8 was hit, but there didn't appear to be an IRP structure in RCX. And for the life of me I can't get the other ones to trigger. Time to read up a bit more on the driver and see wtf else it does. Hmph, so a couple won't trigger no matter what a I do. What a twat. Let's see if there's some other way I can pull out IOCTL codes with WinDBG. Well first let's try this: what do we say to the god of windows?? Reboot today boy, reboot today. Let's reboot and see if it hits a bp. I also see some stuff about setting ACLs poking around IDA so that'll be our next thing if a reboot doesn't do it. And really nothing much from there. This is somewhat surprising as I use this technique and it's my fallback - usually you can just nab em easily from your RE tool. Well anyway, let's see what we can do here. Let's put a breakpoint at some time before our IRP calls and use the "step Over" functionality in windbg and then just check RCX before any function call. That should do it, and if not there's some other tricks we can try (like doing this in Ghidra - it's a bit easier to reverse out IOCTLs from Ghidra sometimes). OK let's get cracking, I'll use local debugging this time. Wait no, that's stupid, i need to breakpoint into my system. Go me, OK net debugging it is.

Alright so far I've gotten 2 IOCTLs:

0x8600204c
0xe281b54f


And the analysis is getting painful and annoying. This used to be quite a simple process when win_driver_plugin was working, as easy as a click and you've got it. I'm not wondering if perhaps it's not worth it to just use my version of win_driver_plugin based on IDA 7.4 and Python 2. But I suppose understanding how this analyis takes place is a great learning experience. That's what I'm going to tell myself or I'm going to throw my computer out the window.
